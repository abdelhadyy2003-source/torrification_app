...
def optimization_function(params, biomass, moisture, size, initial_mass_kg):
    ...
    # Simple cost factor based on process severity
    cost_factor = (T / 320) + (D / 120) 
    
    # Objective: Maximize (Energy Yield * Mass Yield) - Cost Factor * Weight
    objective = (energy_yield * mass_yield) - 0.2 * cost_factor
    
    return -objective # Minimizing the negative of the objective
...
def main():
    ...
    with st.sidebar:
        ...
        header_html = f"""
            <div class="sidebar-header-box">
                {f'<img src="data:image/png;base64,{LOGO_BASE64_STRING}" style="max-width: 80px; margin-bottom: 10px;">' if LOGO_BASE64_STRING else ''}
                <h1>CHEMISCO</h1>
    ...
    profit_delta = (
        (results['yields_mass'].loc['Biochar (Solid Product)', 'Mass (kg)'] * st.session_state.price_biochar_per_kg) 
        - (initial_mass_kg / 1000) * st.session_state.cost_biomass_per_ton
    )
    # Simplified Hourly CAPEX Amortization (Assuming 5 years/300 op days/24 hours) for KPI
    hourly_amortization_rate = st.session_state.capex / (5 * st.session_state.operating_days * 24)
    hourly_opex_base = st.session_state.cost_energy_per_hour
    
    # Calculate batches per hour (for hourly profit KPI)
    batches_per_hour = 60 / duration if duration > 0 else 0
    
    # Net Hourly Profit
    net_hourly_profit = (profit_delta * batches_per_hour) - hourly_opex_base - hourly_amortization_rate 
    
    # Annual Economics for Tabs
    batch_per_day = 60 / duration if duration > 0 else 0
    annual_batches = st.session_state.operating_days * batch_per_day * 24 # 24 hours per day
    ...
    annual_net_profit = annual_revenue - total_annual_opex - (st.session_state.capex/5) # Annual CAPEX amortization over 5 years
    ...
    bfd_html = f"""
    <div class="bfd-container" style="display: flex; justify-content: space-between; align-items: center; text-align: center;">
        <div class="bfd-block" style="border-left: 5px solid #00BCD4;">FEED PREP<p style="color: #00BCD4;">{initial_mass_kg:.0f} kg</p></div>
        <div class="bfd-stream" style="width: 50px; border-top: 2px dashed #66BB6A;"></div>
    ...
    with tab4:
        ...
        df_finance = pd.DataFrame({
            "Metric": ["Annual Revenue (A)", "Annual Feedstock Cost (B)", "Annual Operational Cost (C)", "Total OPEX (B+C)", "Annual CAPEX Amortization (D)", "Annual Net Profit (A - OPEX - D)"],
            "Value": [annual_revenue, annual_feedstock_cost, annual_operational_cost, total_annual_opex, st.session_state.capex/5, annual_net_profit]
        })
        ...
        df_gantt = pd.DataFrame([
            ...
            dict(Task="Procurement & CAPEX", Start='2025-07-01', Finish='2026-03-31', Resource='Phase 1'),
            dict(Task="Construction & Installation", Start='2026-04-01', Finish='2027-03-31', Resource='Phase 2'),
            dict(Task="Commissioning & Ramp-up", Start='2027-04-01', Finish='2027-06-30', Resource='Phase 2'),
            dict(Task="Full Operation", Start='2027-07-01', Finish='2035-12-31', Resource='Operation')
        ])
        
        fig_gantt = px.timeline(df_gantt, x_start="Start", x_end="Finish", y="Task", color="Resource", 
                                color_discrete_sequence=px.colors.qualitative.Bold)
        fig_gantt.update_yaxes(autorange="reversed") 
        fig_gantt.update_layout(paper_bgcolor='#1E1E1E', plot_bgcolor='#1E1E1E', font=dict(color='#E0E0E0'), height=350)
        st.plotly_chart(fig_gantt, use_container_width=True)

    # --- Tab 5: AI Expert ---
    with tab5:
        st.subheader("Chemisco AI Expert Assistant")
        
        if st.session_state['messages'][-1]["content"].lower().startswith("welcome"):
            # Update initial message after first run
            st.session_state["messages"][-1]["content"] = mock_ai_response("summary", results, annual_net_profit, payback_period_years)

        for message in st.session_state.messages:
            with st.chat_message(message["role"]):
                st.markdown(message["content"], unsafe_allow_html=True)

        if prompt := st.chat_input("Ask about thermal loads, project payback, or optimal conditions..."):
    ...
    with tab6:
        st.subheader("Manager Challenge: Achieve the Quality Target!")
        
        current_yield = results['yields_percent'].loc['Biochar (Solid Product)', 'Yield (%)']
        current_ash = results['final_ash_percent']
        
        st.markdown(f"""
        <div class='custom-info-box' style='background-color: #311B92;'>
            <p style='margin: 0; font-weight: bold; color: #E8EAF6;'>🎯 Quality Targets:</p>
            <p style='margin: 5px 0 0; color: #FFFFFF;'>Mass Yield: <b>{st.session_state['target_yield']:.1f}%</b> (Current: {current_yield:.1f}%)</p>
            <p style='margin: 0; color: #FFFFFF;'>Final Ash: <b>{st.session_state['target_ash']:.1f}%</b> (Current: {current_ash:.2f}%)</p>
            <p style='margin: 10px 0 0; font-style: italic; font-size: 14px; color: #C5CAE9;'>Your current inputs must meet or exceed the yield target AND be below the ash target.</p>
        </div>
        """, unsafe_allow_html=True)

        yield_ok = current_yield >= st.session_state['target_yield']
        ash_ok = current_ash <= st.session_state['target_ash']

        if yield_ok and ash_ok:
            st.session_state['has_won'] = True
            st.balloons()
            st.markdown(f"""
            <div class='custom-success-box' style='background-color: #2E7D32;'>
                <h3 style='margin: 0; color: #FFFFFF;'>🏆 Challenge Passed!</h3>
                <p style='margin: 5px 0 0; color: #C8E6C9;'>You achieved the required Biochar quality with these process settings. Your current Annual Net Profit is: <b>${annual_net_profit:,.0f}</b>.</p>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.session_state['has_won'] = False
            st.warning("Keep adjusting the inputs (Temperature, Duration, Feedstock) to meet the yield and ash targets simultaneously.")

        st.markdown("---")
        st.markdown("##### Change Targets")
        col_tg1, col_tg2 = st.columns(2)
        st.session_state['target_yield'] = col_tg1.number_input("Target Mass Yield (%)", min_value=50.0, max_value=100.0, value=st.session_state['target_yield'], step=1.0)
        st.session_state['target_ash'] = col_tg2.number_input("Target Final Ash (%)", min_value=0.5, max_value=50.0, value=st.session_state['target_ash'], step=0.5)
