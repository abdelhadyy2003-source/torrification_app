...
<style>
    ...
    /* Metrics Style (High Contrast) */
    [data-testid="stMetric"] {
        background-color: #E8F5E9; /* Very Light Green Background */
        padding: 15px;
        border-radius: 8px;
        border-left: 5px solid #4CAF50; /* Primary Green Accent */
    ...
    [data-testid="stMetricLabel"] { 
        color: #1B5E20; /* Very Dark Green Label */
        font-weight: 600;
    }
    [data-testid="stMetricDelta"] { 
        color: #0277BD !important; /* Blue Delta for contrast */
        font-weight: bold;
    }
...
def simulate_torrefaction(biomass, moisture, temp_C, duration_min, size, initial_mass_kg, reactor_type="N/A"): 
    ...
    # Calculate mass fraction dynamic change
    current_total_mass_fraction = moisture_curve + volatiles_curve + fixed_carbon_frac_initial + initial_ash_frac
    
    # Calculate Ash concentration (Ash Mass / Total Solid Mass)
    solid_mass_fraction = volatiles_curve + fixed_carbon_frac_initial + initial_ash_frac
    # Prevent division by zero if all mass is gone (shouldn't happen with ash presence)
    ash_concentration_percent = (initial_ash_frac / solid_mass_fraction) * 100 if np.any(solid_mass_fraction > 0) else np.zeros_like(solid_mass_fraction)
    
    final_moisture_loss = initial_moisture_frac # Simplified: assume all initial moisture is eventually lost in the process time frame
    ...
    # Displaying as pseudo-molar percent based on assumed dry gas composition
    ...
    mass_profile = pd.DataFrame({
        "Time (min)": t,
        # Displaying Total Mass % based on initial total mass
        "Total Mass Yield (%)": current_total_mass_fraction * 100,
        # Ash concentration based on total solid mass remaining (Volatiles_rem + Fixed_C + Ash)
...
def generate_pdf_report(results):
    ...
    # 5. Economic Summary (for PDF)
    elements.append(Paragraph("4. Economic Analysis Summary", heading_style))
    
    # Recalculate economic data for PDF (needed here too for completeness)
    p = results["parameters"]
    cost_biomass_per_ton = st.session_state.get('cost_biomass_per_ton', 30.0)
    cost_energy_per_hour = st.session_state.get('cost_energy_per_hour', 5.0)
    price_biochar_per_kg = st.session_state.get('price_biochar_per_kg', 1.20)
    
    cost_feedstock_total = (p['initial_mass'] / 1000) * cost_biomass_per_ton
    hours = p['duration'] / 60
    ...
    economic_data = [
        ["Metric", "Value"],
        ["Revenue (per Batch)", f"${revenue_total:.2f}"],
        ["Feedstock Cost (per Batch)", f"${cost_feedstock_total:.2f}"],
        ["Operational Cost (per Batch)", f"${cost_operations_total:.2f}"],
        ["Total Cost (per Batch)", f"${total_cost:.2f}"],
        ["Net Profit (per Batch)", f"${net_profit:.2f}"],
        ["Return on Investment (ROI)", f"{roi:.1f}%"]
    ]
    t_econ = Table(economic_data, colWidths=[3*inch, 3*inch])
    t_econ.setStyle(TableStyle([
        ...
        ('GRID', (0,0), (-1,-1), 1, colors.grey),
        ('ALIGN', (1,0), (-1,-1), 'CENTER'),
        ('TEXTCOLOR', (1,5), (1,5), colors.green if net_profit >= 0 else colors.red), # Highlight Net Profit
        ('TEXTCOLOR', (1,6), (1,6), colors.green if roi >= 0 else colors.red), # Highlight ROI
    ]))
...
def main():
    # Initialize session state for economic inputs if they don't exist
    if 'cost_biomass_per_ton' not in st.session_state:
        ...
        st.session_state.target_ash = 0 # Placeholder, calculated later
    ...
    with st.sidebar:
        ...
        with st.expander("💰 Cost Management", expanded=False):
            ...
            # Use session state to persist these values
            ...
            st.session_state.cost_energy_per_hour = st.number_input("Operational/Energy Cost ($/hour)", min_value=0.0, value=st.session_state.cost_energy_per_hour, step=0.5, help="Total cost of electricity + labor per hour of operation")
    ...
    if game_mode:
        ...
        # Initialize or update target ash based on initial ash content
        ...
        with col_g2:
            ...
            # Penalty for exceeding max ash is crucial
            ash_penalty = max(0, curr_ash - st.session_state.target_ash) * 50 
            
            score = max(0, 100 - (diff_yield * 2 + diff_ash * 5 + ash_penalty)) # Added strict penalty for high ash
            ...
            if score >= 90:
                ...
                if not st.session_state.has_won:
                 ...
                 st.error("❌ ASH CONTENT EXCEEDED MAX LIMIT! Order Failed.")
            elif score >= 70:
    ...
    with tab3:
        st.subheader("Gas Composition (Molar % in Dry Gas)")
        st.bar_chart(results["gas_composition_molar"])

    with tab4:
        st.subheader("💰 Economic Feasibility Analysis (Per Batch)")
        
        # Recalculate economic data based on session state values
        ...
        # Data for Waterfall
        data_waterfall = [
            ["Initial Cost", total_cost, colors.red],
            ["Revenue", revenue_total, colors.green],
            ["Net Profit", net_profit, colors.grey]
        ]
        
        categories = ["Cost: Feedstock", "Cost: Operational", "Revenue", "Net Profit"]
        measures = ["absolute", "relative", "absolute", "absolute"]
        
        # Calculate intermediate values
        intermediate_values = [
            -cost_feedstock_total, 
        ...
        fig_waterfall = go.Figure(go.Waterfall(
            name = "Cash Flow per Batch", 
            orientation = "v",
            measure = measures,
            x = categories,
            textposition = "outside",
            text = [f"${v:.2f}" for v in intermediate_values],
            y = intermediate_values,
            connector = {"line": {"color": "rgb(63, 63, 63)"}},
            # Custom colors for better visual grouping
            decreasing = {"marker":{"color": "#D32F2F"}}, # Red for negative
            increasing = {"marker":{"color": "#4CAF50"}}, # Green for positive
            totals = {"marker":{"color": "#0277BD"}} # Blue for total
        ))

        fig_waterfall.update_layout(
            title = "Cash Flow Analysis (Per Batch)",
            showlegend = False,
            height = 500,
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(0,0,0,0)'
    ...
    with tab5:
        st.subheader("Generate Professional Report (PDF)")
        st.markdown("""
        هذا التقرير يجمع البيانات الأساسية والجداول والرسومات البيانية في مستند PDF احترافي.
        مثالي لتقديمه كجزء من عرض المشروع أو التوثيق الفني.
        """)
        
        # Ensure latest data is used
        report_buffer = generate_pdf_report(results)
        
        st.download_button(
            label="⬇️ Download Full PDF Report",
            data=report_buffer,
            file_name="Chemisco_Torrefaction_Report.pdf",
